...
export interface CellErr { row: number; col: string; msg: string }
...
export const useDataStore = create<State>((set)=>({
  ...
  setCellErrors:(k,errs)=>set(s=>({cellErrors:{...s.cellErrors,[k]:errs}})),

  addRule:(r)=>set(s=>({rules:[...s.rules,{...r,id:nanoid(6)}]})),
